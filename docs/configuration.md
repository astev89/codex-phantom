...
| `OPENAI_REASONING_EFFORT`        | `medium`                                         | optional                 | Reasoning effort for normal agent/coordinator Responses calls. One of `low`, `medium`, `high`.                                             |
| `OPENAI_MEMORY_REASONING_EFFORT` | `low`                                            | optional                 | Reasoning effort for background memory insight extraction. One of `low`, `medium`, `high`.                                                 |
...
| `EMAIL_SMTP_TLS`                 | `false` for port `587`, `true` for port `465`    | optional                 | Controls implicit TLS for SMTP. Leave unset for STARTTLS on `587`; set explicitly only when a provider requires different behavior.         |
...
| `MEMORY_TOP_K`                   | `12`                                             | optional                 | Positive integer semantic retrieval result count; runtime memory policy bounds clamp this to 1-50.                                         |
| `MEMORY_PER_CATEGORY_LIMIT`      | `3`                                              | optional                 | Positive integer category cap for memory query responses; runtime memory policy bounds clamp this to 1-20.                                 |
| `MEMORY_SUMMARY_LIMIT`           | `2`                                              | optional                 | Positive integer summary count returned with memory; runtime memory policy bounds clamp this to 0-20.                                      |
| `MEMORY_SUMMARY_TRIGGER_COUNT`   | `6`                                              | optional                 | Positive integer threshold for summary generation; runtime memory policy bounds clamp this to 2-50.                                        |
| `MEMORY_SUMMARY_CLUSTER_SIZE`    | `4`                                              | optional                 | Positive integer grouping size for summary generation; runtime memory policy bounds clamp this to 2-50 and no more than the trigger count. |
...
- `OPENAI_MEMORY_REASONING_EFFORT=low`
- `QDRANT_ENABLED=true`
...
Startup fails fast when integer fields are non-positive, reasoning effort values are not `low`, `medium`, or `high`, role/config paths are blank, required secrets are blank, `QDRANT_ENABLED=true` lacks `QDRANT_URL`, production lacks `OPENAI_API_KEY`, or production/default-secret rejection finds `replace-me` or development defaults.
