...
| Governed self-evolution    | Partial         | Durable proposal records, operator APIs, agent proposal tool, approval/rejection, high-risk confirmation, operator-settings apply, rollback metadata, mutation audit, and malformed/direct-apply rejection are implemented. ADR-0003 expands the target to autonomous assignments with delegated autonomous self-evolution. Remaining gaps: autonomous assignment runtime, assignment policy, autonomous mutation ledger, and safe mutation classes for prompts, memory policy, tools, roles, project files, and broader configuration. |
